...
const RecurringRedirect = () => {
    ...
    const processedPaymentIdRef = useRef(null);
        ...
        const fetchPayments = async () => {
            ...
            try {
                ...
                const match = response.data.find((payment) =>
                    String(payment.metadata?.userInfo?.voornaam || "").trim().toLowerCase() === normName &&
                    String(payment.metadata?.userInfo?.email || "").trim().toLowerCase() === normEmail
                );

                if (match) {
                    // Prevent running the full recurring flow twice for same payment
                    if (processedPaymentIdRef.current === match.id) {
                        setLoading(false);
                        return;
                    }
                    processedPaymentIdRef.current = match.id;

                    // Merge full form data from localStorage with trimmed Mollie metadata
                    let savedUserInfo = {};
                    try {
                        const raw = localStorage.getItem('msbc_recurring_userinfo');
                        if (raw) savedUserInfo = JSON.parse(raw);
                    } catch { /* ignore */ }
                    const mollieUi = match.metadata?.userInfo ?? {};
                    const ui = { ...savedUserInfo, ...mollieUi, selectedOption: { ...(savedUserInfo?.selectedOption ?? {}), ...(mollieUi?.selectedOption ?? {}) } };
                    try { localStorage.removeItem('msbc_recurring_userinfo'); } catch { /* ignore */ }

                    // Prevent duplicate inserts in React dev (StrictMode double-invokes effects)
                    if (!didSendRef.current) {
                        didSendRef.current = true;
                        const existingProgramType = ui?.selectedOption?.programType;
                        const resolvedProgramType =
                            existingProgramType && String(existingProgramType).trim().length > 0
                                ? existingProgramType
                                : (ui?.selectedOption?.recurring ? "club" : undefined);
                        const dataToSend = {
                            ...ui,
                            status: match.status,
                            molliePaymentId: match.id,
                            mollieCustomerId: match.customerId || "",
                            selectedOption: {
                                ...(ui.selectedOption ?? {}),
...

                {
                    (loading || paymentStatus === null) && (
                        <div className="text-center text-gray-700">
                            <p className="text-lg font-semibold">Bezig met verwerken van je betaling...</p>
                            <p className="text-sm mt-2">Een ogenblik geduld, we controleren je abonnementsstatus.</p>
                        </div>
                    )
                }
